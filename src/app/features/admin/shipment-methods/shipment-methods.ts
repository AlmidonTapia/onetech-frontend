...
import { ShipmentMethod, CreateShipmentMethodRequest } from '../../../core/models/shipment.model';
import { CurrencyPenPipe } from '../../../shared/pipes/currency-pen.pipe';

@Component({
  selector: 'app-shipment-methods',
  standalone: true,
  imports: [CommonModule, TableModule, ReactiveFormsModule, ButtonComponent, CardComponent, DialogModule, CurrencyPenPipe],
  template: `
          ...
          <ng-template pTemplate="body" let-method>
            <tr>
              ...
              <td class="text-muted" style="text-align: center;">
                <small>{{ method.idShipmentMethod.substring(0,8) }}...</small>
              </td>
            </tr>
          </ng-template>
          <ng-template pTemplate="emptymessage">
            <tr>
              <td colspan="3" class="empty-state">No hay métodos de envío registrados.</td>
            </tr>
          </ng-template>
        </p-table>
      </div>
    ...
    <p-dialog [visible]="formVisible()" (visibleChange)="formVisible.set($event)" [header]="'Nuevo Método de Envío'" [style]="{width: '400px'}" [modal]="true">
      <form [formGroup]="form" (ngSubmit)="onSave()" class="modal-form">
        <div class="form-grid">
          <div class="form-group">
            <label>Nombre del Método</label>
            <input type="text" formControlName="methodName" class="form-input" placeholder="Ej: Express" />
          </div>
          <div class="form-group">
...
export class ShipmentMethodsComponent implements OnInit {
  ...
  onSave() {
    ...
    const data: CreateShipmentMethodRequest = {
      methodName: this.form.value.methodName!,
      basePrice: this.form.value.basePrice!
    };

    this.shipmentService.createMethod(data).subscribe({
      next: () => {
