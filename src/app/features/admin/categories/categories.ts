...
export class CategoriesComponent implements OnInit {
  ...
  loadCategories(event?: any) {
    ...
    this.categoryService.getAll(page, this.apiConfig.pageSize, this.searchTerm()).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: r => {
  ...
  onSave(data: CreateCategoryRequest) {
    ...
    request$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
        this.alertService.success('Categoría guardada');
  ...
  onDelete(c: Category) {
    this.modalService.open({
      ...
      onConfirm: () => {
        this.categoryService.delete(c.idCategory).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
          next: () => {
