...
@Component({
  ...
  template: `
    <a [href]="waLink()" target="_blank" rel="noopener noreferrer"
      ...
      <span class="wa-label">{{ariaLabel}}</span>
    </a>
  `,
  styles: [`
    .wa-btn {
...
export class WhatsappBtnComponent {
  ...
  waLink = computed(() => {
    ...
    const url = this.productUrl ?? (isBrowser ? window.location.href : '');
    
    let msg = `Hola, tengo una consulta sobre la tienda OneTech. (${url})`;
    if (this.productName) {
      msg = `Hola, estoy interesado en el producto: ${this.productName} ${url}`;
    }
